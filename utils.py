...
def main(grp_size):
    ...
    # Update group information for each user
    with ThreadPoolExecutor(max_workers=8) as executor:
        executor.map(lambda user: user.update_group(users), users)

    # Step 1: User Setup and Mask Generation
    with ThreadPoolExecutor(max_workers=8) as executor:
        executor.map(lambda user: user.generate_inter_user_mask(), users)

    # Step 2: Masking and Data Sharing
    with ThreadPoolExecutor(max_workers=8) as executor:
        executor.map(lambda user: user.share_masks(), users)

    # Calculate masked value for each user
    with ThreadPoolExecutor(max_workers=8) as executor:
        executor.map(lambda user: user.calculate_masked_value(), users)
